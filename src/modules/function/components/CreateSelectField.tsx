import { FilledInput, MenuItem, Select, Typography } from '@mui/material';
...
const SelectField = (props: Props) => {
    const { label, control, name, require, placeholder, data } = props
    return (
        <Grid2 sx={{ flexFlow: "row wrap", alignItems: 'center', justifyContent: 'space-between' }} container spacing={1}>
            ...
            <Grid2 xs={12} sm={12} md={7} lg={7.2} xl={8}>
                <Controller
                    ...
                    control={control}
                    defaultValue=''
                    rules={{ required: require, validate: (value) => value !== 0 }}
                    render={({ field }) => (
                        <Select
                            {...field}
                            name={name}
