...
import { ChangeTitle } from '../../../models/todo';


function TodoPage() {
  const [reset, setReset] = useState<boolean>(false);
  const [swithTitle, setSwithTitle] = useState<Array<ChangeTitle>>([]);
  const dispatch = useDispatch();
  const listPhoto = useSelector((state: AppState) => state.todo.listPhoto);
  const loading = useSelector((state: AppState) => state.todo.loading);
  const hasMore = useSelector((state: AppState) => state.todo.hasMore);
  ...
  const handleChangeConfirm = useCallback((title: string, id: number, xitem?: boolean) => {
    setSwithTitle((prev) => {
      ...
      if (check && !xitem) {
        const newData: ChangeTitle[] = prev.map((el) => {
          if (id === el.id) {
            return {
              ...el,
              title: title,
          ...
          <div style={{ border: '1px solid #f0f0f0', padding: '30px', borderRadius: '6px' }}>
            <div className="d-flex justify-content-center flex-column">
              ...
              {listPhoto.map((data, index) => {
                ...
                return (
                  <Fragment key={index}>
                    <TodoForm data={data} reset={null} handlech={null} handleChangeConfirm={handleChangeConfirm} />
                  </Fragment>
                );
              })}
              {loading && (
